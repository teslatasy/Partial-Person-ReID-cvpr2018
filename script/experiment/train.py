...
from tri_loss.model.PartialModel import PartialModel
from tri_loss.model.TripletLoss import TripletLoss
from tri_loss.model.loss import global_loss
from tri_loss.model.loss import normalize
...
class Config(object):
  def __init__(self):
    ...
    parser.add_argument('--partial_dataset', type=str, default='Partial REID',
                        choices=['Partial_REID', 'Partial_iLIDS', 'holistic'])
    parser.add_argument('--spatial_train', type=str2bool, default=False)
    parser.add_argument('--resize_h_w', type=eval, default=(384, 192))
    ...
    parser.add_argument('--exp_dir', type=str, default='Dataset/')
    ...
    self.train_set_kwargs = dict(
      ...
      mirror_type=self.train_mirror_type, testim = 'holistic',
      prng=prng)
    ...
    self.val_set_kwargs = dict(
      ...
      mirror_type=self.test_mirror_type, testim = 'holistic',
      prng=prng)
    ...
    self.test_set_kwargs = dict(
      ...
      final_batch=self.test_final_batch,
      shuffle=self.test_shuffle,
      mirror_type=self.test_mirror_type, testim = self.partial_dataset,
      prng=prng)
  ...
  for ep in range(start_ep, cfg.total_epochs):
    ...
    while not epoch_done:
      ...
      loss, p_inds, n_inds, dist_ap, dist_an, dist_mat = global_loss(
        tri_loss, feat, spatialFeature, labels_t, cfg.spatial_train,
