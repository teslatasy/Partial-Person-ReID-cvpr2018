...
from .Partial_REID_test import Partial_REID_test
from .Partial_iLIDS_test import Partial_iLIDS_test
...
def create_dataset(
  ...
  assert testim in ['Partial_REID', 'Partial_iLIDS', 'holistic'], \
    "Unsupported Dataset Part {}".format(part)
  ########################################
  ...
  if part == 'trainval':
    ...
    kwargs.update(cmc_kwargs)
    if testim == 'Partial_REID':
      ret_set = Partial_REID_test(
        im_dir=im_dir,
        im_names=im_names,
        marks=marks,
        **kwargs)
    elif testim =='Partial_iLIDS':
      ret_set = Partial_iLIDS_test(
        im_dir=im_dir,
        im_names=im_names,
        marks=marks,
        **kwargs)
  ...
  if testim == 'Partial_REID':
    print('-' * 40)
    print('{} {} set'.format('Partial REID', part))
    print('-' * 40)
    print('NO. Images: {}'.format(600))
    print('NO. IDs: {}'.format(60))

    try:
      print('NO. Query Images: {}'.format(300))
      print('NO. Gallery Images: {}'.format(300))
    except:
      pass
  elif testim == 'Partial_iLIDS':
    print('-' * 40)
    print('{} {} set'.format('Partial iLIDS', part))
    print('-' * 40)
    print('NO. Images: {}'.format(238))
    print('NO. IDs: {}'.format(119))

    try:
      print('NO. Query Images: {}'.format(119))
      print('NO. Gallery Images: {}'.format(119))
    except:
      pass
  else:
    print('-' * 40)
