...
class Partial_REID_test(Dataset):
    """
  Args:
  ...
  """
    ...
    def __init__(
        ...
        super(Partial_REID_test, self).__init__(dataset_size=len(im_names), **kwargs)
    ...
    """
        ...
        with measure_time('Extracting feature...', verbose=verbose):
            ...
            Probe, Probe1 = self.extract_feat(
                labels, 0)
        Probe = np.vstack(Probe)
        Probe1 = np.vstack(Probe1)

        # print(Probe1[0][0,:,:].shape)
        with measure_time('Extracting feature...', verbose=verbose):
            ...
            for line in fh:
                ...
                words = line.split()
                labels.append(words[0])
            Gallery, Gallery1 = self.extract_feat(
                labels, 1)
        Gallery = np.vstack(Gallery)
        Gallery1 = np.vstack(Gallery1)
            ...
            for j in range(0, 5):
                ...
                for i in range(0, 5):
                    ...
                    for k in range(i, 300, 5):
                        SpatialGallery.append(Gallery1[k])
                    marks = []
                    for iii in range(0, 300):
                        if (iii % 5 == i):
                            marks.append(1)
        ...
        with measure_time('Computing scores...', verbose=verbose):
            for lam in range(0, 11):
                ...
                cmc_scores1 = []
                weight = lam * 0.1
                for i in range(0, len(global_dist)):
                    q_g_dist = (1 - weight) * global_dist[i] + weight * spatial_dist[i]
                    mAP, cmc_scores = compute_score(q_g_dist)
                    cmc_scores1.append(cmc_scores)
                    mAP1.append(mAP)
                print('{:<30}'.format('Single Query:'), end='')
                print_scores(sum(mAP1) / 25, sum(cmc_scores1) / 25)
