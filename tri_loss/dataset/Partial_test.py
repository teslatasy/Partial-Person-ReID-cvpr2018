...
class Partial_test(Dataset):
  ...
  """
    ...
    def __init__(
        ...
        super(Partial_test, self).__init__(dataset_size=len(im_names), **kwargs)
    ...
    """
        ...
        with measure_time('Computing distance...', verbose=verbose):
            ...
            for lam in range(0, 11):
                ...
                q_g_dist = (1 - weight) * q_g_global_dist + weight * q_g_sppatial_dist #(1-w)*global_dist + w* spatial_dist
                mAP, cmc_scores = compute_score(q_g_dist)
                cmc_scores1.append(cmc_scores)
                mAP1.append(mAP)
                print('{:<30}'.format('Single Query:'), end='')
                print_scores(sum(mAP1), sum(cmc_scores1))